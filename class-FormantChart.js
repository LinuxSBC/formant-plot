function FormantChart(parameters, elementId) {
    ...
    this.drawTrapezoid = function () {
        var command = "M" + this.plotLeft() + "," + this.plotTop() + "H" + this.plotRight() + "V" + this.plotBottom() + "H" + (this.plotRight() - this.p.trapezoidRatio * this.plotWidth()) + "L" + this.plotLeft() + "," + this.plotTop();
    ...
    this.drawHorizontalLines = function () {
        ...
        for (var i = 1; i <= this.p.horizontalLines; i++) {
            ...
            var command = "M" + right + "," + (this.plotTop() + y) + "L" + left + "," + (this.plotTop() + y);
            var p = this.paper.path(command);
    ...
    this.drawVerticalLines = function () {
        ...
        for (var i = 1; i <= this.p.verticalLines; i++) {
            var command = "M" + (this.plotRight() - i * intervalSize) + "," + this.plotTop() + "L" + (this.plotRight() - i * intervalSize * this.p.trapezoidRatio) + "," + this.plotBottom();
            var p = this.paper.path(command);
            p.attr("stroke-width", this.p.gridLineWidth);
            p.attr("stroke", this.p.gridLineColor);
        }
